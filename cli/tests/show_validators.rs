...
async fn test_show_validators_display_output() {
    ...
    // Replace the warning emoji with its ASCII fallback for readable comparison
    let output = output.replace("\u{26a0}\u{fe0f}", "!");
    let expected = concat!(
        "   Identity                                      Vote Account                              Commission  Last Vote        Root Slot     Skip Rate  Credits  Version Client Id           Active Stake\n",
        "! 7RoSF9fUmdphVCpabEoefH81WwrW7orsWonXWqTXkKV8  7RoSF9fUmdphVCpabEoefH81WwrW7orsWonXWqTXkKV8  100.00%          -                -          -           0  unknown         0        0.000000000 SOL (NaN%)\n",
        "\n",
        ...
        "\n",
        "Stake By Client ID:\n",
        "      0 -    0 current validators (  NaN%)   1 delinquent validators (  NaN%)\n",
    );
