...
pub(crate) mod tests {
    ...
    fn test_migrate_builtin_e2e_init_after_successful_migration() {
        ...
        let mut bank = Bank::new_with_config_for_tests(
            &genesis_config,
            BankTestConfig {
        ...
        // Run `compute_and_update_feature_set` and
        // `update_transaction_processor` to simulate starting up from a
        // snapshot. Clear all builtins to simulate a fresh bank init.
        bank.transaction_processor
            .global_program_cache
            .write()
            .unwrap()
