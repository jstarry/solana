use crate::{
    ...
    message::{MessageHeader, MESSAGE_VERSION_PREFIX, legacy::BUILTIN_PROGRAMS_KEYS},
    pubkey::Pubkey,
...
impl Message {
    ...
    fn is_writable_index(&self, key_index: usize) -> bool {
        ...
        if key_index >= num_account_keys {
            ...
            let num_writable_dynamic_addresses = self.address_table_lookups.iter().map(|lookup| lookup.writable_indexes.len()).sum();
    ...
    pub fn is_maybe_writable(&self, key_index: usize) -> bool {
        let mut is_maybe_writable = self.is_writable_index(key_index);
        if let Some(key) = self.account_keys.get(key_index) {
            ...
            is_maybe_writable = !(sysvar::is_sysvar_id(key)
    ...
    pub fn is_signer(&self, key_index: usize) -> bool {
        key_index < usize::from(self.header.num_required_signatures)
    }
}
