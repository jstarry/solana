...
use {
    crate::accounts::RentDelinquentAccount,
    solana_sdk::{
...
impl RentCollector {
    ...
    //  the amount of rent collected and
    //  a record of the account if it was delinquent
    ...
    ) -> (u64, Option<RentDelinquentAccount>) {
            ...
            )
        {
            ...
            if exempt || rent_due != 0 {
                if account.lamports() > rent_due {
                    ...
                    let delinquent_account = Some(RentDelinquentAccount {
                        key: *address,
                        cleared_owner: *account.owner(),
                        cleared_data_len: account.data().len(),
                    });
                    *account = AccountSharedData::default();
                    (rent_charged, delinquent_account)
                }
            } else {
...
mod tests {
    ...
    fn test_collect_from_account_created_and_existing() {
        ...
        let (collected, delinquent_account) = rent_collector.collect_from_created_account(
        ...
        assert!(delinquent_account.is_none());

        // collect rent on a already-existing account
        let (collected, delinquent_account) = rent_collector.collect_from_existing_account(
            &solana_sdk::pubkey::new_rand(),
    ...
    fn test_rent_exempt_temporal_escape() {
        ...
        let (collected, delinquent_account) =
    ...
    fn test_rent_exempt_sysvar() {
        ...
        assert!(delinquent_account.is_some());
    }
}
