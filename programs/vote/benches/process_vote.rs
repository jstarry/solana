...
use {
    ...
    solana_vote_program::{
        vote_instruction::{
            breakpoint_key_1, breakpoint_key_2, breakpoint_key_3, breakpoint_key_4,
            breakpoint_key_5, load_sysvar_key, VoteInstruction,
        },
...
fn do_bench(bencher: &mut Bencher, feature: Option<Pubkey>) {
    ...
    let last_vote_slot = num_initial_votes + num_vote_slots - 1;
    ...
    let instruction_accounts = instruction
        ...
        .iter()
        .enumerate()
        .map(|(index_in_transaction, account_meta)| {
            InstructionAccount {
...
#[bench]
fn bench_process_vote_instruction_sysvar_cache(bencher: &mut Bencher) {
    do_bench(bencher, None);
}

#[bench]
fn bench_process_vote_instruction_master(bencher: &mut Bencher) {
    do_bench(bencher, Some(load_sysvar_key()));
}

#[bench]
fn bench_process_vote_instruction_return_immediately(bencher: &mut Bencher) {
    do_bench(bencher, Some(breakpoint_key_1()));
}

#[bench]
fn bench_process_vote_instruction_return_after_ix_deser(bencher: &mut Bencher) {
    do_bench(bencher, Some(breakpoint_key_2()));
}

#[bench]
fn bench_process_vote_instruction_return_after_get_sysvars(bencher: &mut Bencher) {
    do_bench(bencher, Some(breakpoint_key_3()));
}

#[bench]
fn bench_process_vote_instruction_return_after_vote_state_deser(bencher: &mut Bencher) {
    do_bench(bencher, Some(breakpoint_key_4()));
}

#[bench]
fn bench_process_vote_instruction_return_after_push_votes(bencher: &mut Bencher) {
    do_bench(bencher, Some(breakpoint_key_5()));
}
