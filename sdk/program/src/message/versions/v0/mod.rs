...
use crate::{
    ...
    message::{
        compiled_keys::{compile_instructions, AddressLookupTable, CompiledKeys},
        legacy::BUILTIN_PROGRAMS_KEYS,
...
impl Message {
    ...
    pub fn compile(
        ...
        address_lookup_tables: &[AddressLookupTable],
        blockhash: &Hash,
    ) -> Self {
        let mut compiled_keys = CompiledKeys::compile(Some(payer), instructions);

        // for each lookup table, create lookups and a list of corresponding keys
        // this relies on lookups and addresses to be aligned.
        let (address_table_lookups, loaded_addresses) = address_lookup_tables
            .iter()
            .filter_map(|lookup_table| compiled_keys.drain_lookups(lookup_table))
            .unzip();
        let (header, static_account_keys) = compiled_keys.into_header_and_account_keys();

        let account_keys = AccountKeys::new(&static_account_keys, Some(&loaded_addresses));
        let instructions = compile_instructions(instructions, account_keys.iter());
