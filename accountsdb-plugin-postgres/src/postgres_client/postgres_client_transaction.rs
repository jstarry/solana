...
use {
    ...
    solana_sdk::{
        ...
        message::{
            v0::{self, AddressTableLookup, LoadedAddresses},
...
#[derive(Clone, Debug, ToSql)]
#[postgres(name = "AddressTableLookup")]
pub struct DbAddressTableLookup {
...
pub struct DbTransactionMessageV0 {
    ...
    pub recent_blockhash: Vec<u8>,
    pub instructions: Vec<DbCompiledInstruction>,
    pub address_table_lookups: Vec<DbAddressTableLookup>,
}
...
impl From<&AddressTableLookup> for DbAddressTableLookup {
    fn from(lookup_indexes: &AddressTableLookup) -> Self {
        Self {
            writable_indexes: lookup_indexes
                .writable_indexes
                .iter()
                .map(|address_idx| *address_idx as i16)
                .collect(),
            readonly_indexes: lookup_indexes
                .readonly_indexes
                .iter()
                .map(|address_idx| *address_idx as i16)
                .collect(),
...
fn build_db_transaction(slot: u64, transaction_info: &ReplicaTransactionInfo) -> DbTransaction {
    DbTransaction {
        ...
        v0_loaded_message: match transaction_info.transaction.message() {
            SanitizedMessage::V0(mapped_message) => Some(DbLoadedMessageV0::from(mapped_message)),
            _ => None,
        },
...
pub(crate) mod tests {
    ...
    fn check_address_table_lookups_equality(
        ...
        db_address_table_lookups: &DbAddressTableLookup,
    ) {
        assert_eq!(
    ...
    fn test_transform_address_table_lookups() {
        let address_table_lookups = AddressTableLookup {
            writable_indexes: vec![1, 2, 3],
            readonly_indexes: vec![4, 5, 6],
        };

        let db_address_table_lookups = DbAddressTableLookup::from(&address_table_lookups);
