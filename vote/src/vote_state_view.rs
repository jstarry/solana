use {
    ...
    core::fmt::Debug,
    field_frames::AuthorizedVotersListFrame,
...
impl VoteStateView {
    ...
    pub fn epoch_credits_iter(&self) -> impl Iterator<Item = (Epoch, u64, u64)> + '_ {
        self.epoch_credits_view().into_iter().map(|item| {
            let mut cursor = std::io::Cursor::new(item);
            let epoch = solana_serialize_utils::cursor::read_u64(&mut cursor).unwrap();
            let credits = solana_serialize_utils::cursor::read_u64(&mut cursor).unwrap();
            let prev_credits = solana_serialize_utils::cursor::read_u64(&mut cursor).unwrap();
    ...
    pub fn credits(&self) -> u64 {
        self.epoch_credits_view()
            ...
            .map(|item| {
                let mut cursor = std::io::Cursor::new(item);
                cursor.set_position(core::mem::size_of::<Epoch>() as u64);
...
mod tests {
    ...
    fn assert_eq_vote_state_v3(vote_state_view: &VoteStateView, vote_state: &VoteState) {
        ...
        let view_credits = vote_state_view.epoch_credits_iter().collect::<Vec<_>>();
